...
![](assets/originals/1-Joyride.png)
...
![](assets/originals/2-Time-Space-Complexity.png)
...
<p><a href="https://my.generalassemb.ly/software-engineering-immersive-7d-31878/software-engineering-immersive-class-work-3f70d8/units/algorithms/lessons/big-o-notation-612789/activity/7c1dfdc1-1e95-4c3f-90fe-113137d7e409?wvideo=pbcavxigqp"><img src="https://embed-ssl.wistia.com/deliveries/b132f53a70d63bbb87b0e82faad4d182a4584411.jpg?image_play_button_size=2x&amp;image_crop_resized=960x540&amp;image_play_button=1&amp;image_play_button_color=017991e0" width="400" height="225" style="width: 400px; height: 225px;"></a></p><p><a href="https://my.generalassemb.ly/software-engineering-immersive-7d-31878/software-engineering-immersive-class-work-3f70d8/units/algorithms/lessons/big-o-notation-612789/activity/7c1dfdc1-1e95-4c3f-90fe-113137d7e409?wvideo=pbcavxigqp">7 of 28 | Big O Notation | Algorithms | myGA</a></p>

### Knowledge Check

<details open>
<summary>How did algorithmic complexity affect the user experience for Drake’s app?</summary>
<br>
...
## Highly Efficient Algorithm Type: Constant Complexity [O(1)]

Constant Complexity algorithms fall into the ‘highly efficient’ category. They have a constant space (memory usage) and time (number of steps needed) complexity, no matter the size of the input.
...
![](assets/originals/4-Peeling-Apples.png)

### What O(N) Looks Like in Code
...
In each, we go through the array and perform an action with each item in it. If we have the array [1], each will execute once. If we have the array [3, 5, 1000] the code will run 3 times. If our array has 1000 items, the code will execute 1000 times!
...
## Highly Efficient Algorithm Type: Logarithmic Complexity [O(log (n)]
...
Imagine flipping through a phonebook to find a someone’s number. You could start at the beginning of the phone book and read every name on every page until it found the name you're looking for. Sounds awful, right? That’s a linear `(O(N))` algorithmic approach.
...
![](assets/originals/5-Phone-Book.png)

### What O(log (n)) Looks Like in Code

The phonebook example is an illustration of binary search, which is a classic `O(log (N))` algorithm. In an unsorted array, if we want to find the index of an item with a given value, we have to iterate through it and check if each item is equal to the item we are searching for. However, if we know that we have a sorted array, we can do this a lot easier!

The binary search algorithm looks like this:
...
Let’s say we want to find the value `5` in this array `[1, 3, 5, 7, 9, 11, 13]`. The above function would run 3 times instead of the 7 that we would need if we iterated through the entire array! This logarithmic algorithm is super efficient, because even if we have a million items in our array, on average we will only need to execute the binary search 20 times.

## Inefficient Algorithm Type: Factorial Complexity [O(N!) and O(2^N)]
...
Sometimes this complexity category can't be avoided, but they should bring up some red flags.
...
![](assets/originals/8-Input-Size-Run-Time-Graph-Constant.png)

#### Logarithmic Complexity O(log(N))

![](assets/originals/9-Input-Size-Run-Time-Graph-Logarithmic.png)

#### Linear Complexity O(N)

![](assets/originals/6-Input-Size-Run-Time-Graph-Linear.png)

#### Quadratic Complexity O(N^2)

![](assets/originals/10-Input-Size-Run-Time-Graph-Quadratic.png)

#### Factorial Complexity O(N!)

![](assets/originals/7-Input-Size-Run-Time-Graph-Factorial.png)
...
<br>
You can run a binary search, which cuts the runtime in half at each step, giving you Logarithmic complexity.
</details>
...
<br>
Correct answer: 4. You can run binary search, which cuts the runtime in half at each step, which gives you logarithmic complexity.
</details>
...
<p><a href="https://my.generalassemb.ly/software-engineering-immersive-7d-31878/software-engineering-immersive-class-work-3f70d8/units/algorithms/lessons/big-o-notation-612789/activity/e51253c5-45d7-495c-bc5a-040ce36023ce?wvideo=k506s0fgsz"><img src="https://embed-ssl.wistia.com/deliveries/5cbac5361e58fad5edcccac6e1a44631345ac367.jpg?image_play_button_size=2x&amp;image_crop_resized=960x540&amp;image_play_button=1&amp;image_play_button_color=017991e0" width="400" height="225" style="width: 400px; height: 225px;"></a></p><p><a href="https://my.generalassemb.ly/software-engineering-immersive-7d-31878/software-engineering-immersive-class-work-3f70d8/units/algorithms/lessons/big-o-notation-612789/activity/e51253c5-45d7-495c-bc5a-040ce36023ce?wvideo=k506s0fgsz">27 of 28 | Big O Notation | Algorithms | myGA</a></p>
